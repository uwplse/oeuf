...
Lemma fsim_simulation':
  ...
  \/ (exists i', fsim_order S i' i /\ S i' s1' s2).
Proof.
...
Lemma simulation_plus:
  ...
  forall i s2, S i s1 s2 ->
  (exists i', exists s2', Plus L2 s2 s2' /\ S i' s1' s2')
  \/ (exists i', clos_trans _ (fsim_order S) i' i /\ S i' s1' s2).
Proof.
...
(* simulation *)
  ...
  exploit (fsim_simulation' S12); eauto. intros [[i1' [s3' [C D]]] | [i1' [C D]]].
...
Lemma bsim_simulation':
  ...
  \/ (exists i', bsim_order S i' i /\ S i' s1 s2').
Proof.
